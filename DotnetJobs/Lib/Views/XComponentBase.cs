...
using DotnetJobs.Lib.Validation;

namespace DotnetJobs.Lib.Views;
...
public class XComponentBase : IComponent
{
    ...
    [Inject] SessionManager SessionManager  { get; set; }
    ...
    {
        if (!HasErrors || Errors is null) return false;

        var error = Errors.FirstOrDefault(x => x.MemberName == fieldName);
